...
def login(jaccount_name: str, jaccount_password: str) -> requests.Session:
    try:
        ...
        url: str = 'https://i.sjtu.edu.cn/jaccountlogin'
        headers: dict = {
            ...
            if cookie.get('JAAuthCookie') is not None:
                ...
                if '&err=' in response.url:
                    if '&err=1' in response.url:
                        ...
                        print(
                            '[ERROR/LOGIN] YOU HAS CHANGED YOUR PASSWORD. CURRENT SESSION REQUIRE YOU TO RETRY LOGIN.')
        ...
        #     if '&err=' in response.url:
        #         print('[ERROR/LOGIN] AN ERROR OCCURRED WHILE LOGGIN IN. CONSIDER A WRONG USER NAME OR PASSWORD,',
        #               'OR A WEAK NETWORK QUALITY.')
        #         return {} #
